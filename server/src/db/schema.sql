...
CREATE TABLE IF NOT EXISTS users (
  ...
  notif_prefs  JSONB NOT NULL DEFAULT '{"push":true,"whatsapp":true,"email":false}'::jsonb,
  created_at   TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at   TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add columns to pre-existing databases (idempotent)
ALTER TABLE users ADD COLUMN IF NOT EXISTS notif_prefs JSONB NOT NULL
  DEFAULT '{"push":true,"whatsapp":true,"email":false}'::jsonb;
...
-- ── join_requests (pending invites awaiting Head approval) ────
...
CREATE INDEX IF NOT EXISTS idx_jr_group_status ON join_requests(group_id, status);
CREATE INDEX IF NOT EXISTS idx_jr_user ON join_requests(user_id, status);

-- ── WhatsApp bot: the circle the bot currently acts on for a user ──
-- (added here because it references groups, which is created above)
ALTER TABLE users ADD COLUMN IF NOT EXISTS bot_group_id UUID
...
DROP TRIGGER IF EXISTS set_timestamp_groups ON groups;
