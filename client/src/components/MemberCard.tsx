...
export default function MemberCard({
  ...
  const { speak, stop, isSpeaking, isSupported, readMemberRow } = useTTS()
  ...
  return (
    <li
      ...
      `}
      role="listitem"
      aria-label={`${member.name}, ${location?.locality ?? 'location unknown'}, ${distance ?? ''} km`}
      tabIndex={0}
      onClick={onClick}
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-3 text-xs" aria-live="polite">
          {distance && (
            <span className="text-teal-light font-medium">{distance} km</span>
          )}
          {eta && (
            <span className="text-gray-400">🚗 {eta} min</span>
          )}
